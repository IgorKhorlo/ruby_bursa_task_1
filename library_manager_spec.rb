...
describe LibraryManager do
  ...
  context '#author_translit' do
    ...
    it 'return name in transliteration, original = Ігор Хорло' do
      author_name = "Ігор Хорло"

      res = LibraryManager.new.author_translit(author_name)

      expect(res).to eq 'Ihor Khorlo'
    end

    it 'return name in transliteration, original = Альона Булана' do
      author_name = "Альона Булана"

      res = LibraryManager.new.author_translit(author_name)

      expect(res).to eq 'Alona Bulana'
    end
    ...
    it 'should return correctly transliterated from http://kyivpassport.com/transliteratio/' do
      s = <<-TEXT
<td valign="bottom" nowrap="nowrap" width="113">Алушта</td>
...
TEXT
      ukr, transl = *s.gsub(%r@<td.*?>(.*?)</td>@, ' \1 ').gsub(/\s{2,}/, ' ').split.partition.each_with_index { |obj, i| i.even? }
      ukr.each_with_index do |ukr_name, i|
        res = LibraryManager.new.author_translit(ukr_name)
        expect(res).to eq transl[i]
      end
    end
